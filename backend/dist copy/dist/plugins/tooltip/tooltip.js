...
(function (factory) {
  ...
  $(document).on("click", function (event) {
    ...
    if (
      $target.hasClass(tooltipClasses.base) ||
      ($target.closest("." + tooltipClasses.item).length &&
        !$target.hasClass(tooltipClasses.close))
    )
      return;
  ...
  var methods = {
    ...
    init: function (options) {
      ...
      var userOptions = $.extend(
        ...
        {},
        options,
        methods.getAttrOptions(self)
      );
      var currentOptions = $.extend(
        true,
        {},
        methods.getDefaultOptions(),
        userOptions
      );
      ...
      if (methods.stringToBoolean(currentOptions.fromTitle)) {
        ...
        var html = methods.getHtmlTemplate(
    ...
    create: function (data) {
      ...
      if (
        !methods.isEmptyObjectProperty(id) ||
        current.hasClass(data.options.ignoreClass)
      )
        return;
      ...
      var tooltip = $(
        '<div style="display: none;" data-mytooltip-id="' +
          id +
          '" className="mytooltip system-mytooltip--' +
          options.action +
          " " +
          tooltipClasses.item +
          " " +
      ...
      if (options.hoverTooltip) {
        tooltip.append(
          '<div className="mytooltip-backing ' + tooltipClasses.backing + '">'
        );
      }
    ...
    hide: function (tooltip, options) {
      ...
      var base = $(
        "." + tooltipClasses.base + "[data-mytooltip-id='" + id + "']"
      );
    ...
    setPosition: function (tooltip, data) {
      ...
      var animateOffsetPx = options.animateOffsetPx
        ? parseInt(options.animateOffsetPx)
        : 0;
      ...
      function setSizeTooltip() {
        ...
        sizeTooltip.width = options.widthOfParent
          ? (sizeElement.width / 100) * parseInt(options.widthOfParent)
          : tooltip.outerWidth();
        callSwith();
      }
      ...
      function callSwith() {
        ...
        switch (options.direction) {
          case "top":
            tooltip.css({
              left:
                position.left + sizeElement.width / 2 - sizeTooltip.width / 2,
              top:
                position.top -
                sizeTooltip.height -
                +options.offset -
                animateOffsetPx +
                +offsetVerticalBorder,
            });
            sizeBacking =
              position.top -
              parseInt(tooltip.css("top")) -
              sizeTooltip.height -
              animateOffsetPx;
            backing.css({
              height: sizeBacking,
          ...
          case "right":
            tooltip.css({
              left:
                ...
                +options.offset +
                animateOffsetPx -
                +offsetHorizontalBorder,
              top:
                position.top - sizeTooltip.height / 2 + sizeElement.height / 2,
            });
            sizeBacking =
              parseInt(tooltip.css("left")) -
              position.left -
              sizeElement.width -
              animateOffsetPx;
            backing.css({
      ...
      switch (action) {
        ...
        case "focus":
          ...
          current.on(actionGet, function (event) {
            if (
              !$(event.relatedTarget).is(
                "." + tooltipClasses.item + "," + "." + tooltipClasses.backing
              )
            ) {
              methods.create(data);
            }
          ...
          current.on(actionLose, function (event) {
            ...
            if (
              ...
              !methods.stringToBoolean(options.hoverTooltip)
            ) {
              methods.hide();
            } else if (
              !$(event.relatedTarget).is(
    ...
    getAttrOptions: function (current) {
      ...
      for (var option in defaultOptions) {
        ...
        var dataAttrValue = $(current).attr(
          "data-mytooltip-" + currentAttrName
        );
    ...
    callEvents: function (current, event) {
      ...
      if (id >= 0) {
        content = $(
          "." + tooltipClasses.item + '[data-mytooltip-id="' + id + '"]'
        );
      }
      ...
      if (id !== undefined) {
        ...
        for (var block in tooltipsStorage) {
          if (tooltipsStorage.hasOwnProperty(block)) {
            if (
              $self.data("mytooltip-id") ===
              $(tooltipsStorage[block].current).data("mytooltip-id")
            ) {
