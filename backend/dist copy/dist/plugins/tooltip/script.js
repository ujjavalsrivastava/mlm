...
$(document).ready(function () {
  ...
  $(".add-element").on("click", function (event) {
    event.preventDefault();
    $(this).after(
      '<br><div style="margin: 10px 0;" className="js-mytooltip type-inline-block style-block style-block-three"' +
        ' data-mytooltip-animate-duration="150" data-mytooltip-direction="left" data-mytooltip-custom-className="align-center"' +
  ...
  $(document).on("click", ".add-to-basket", function (event) {
    ...
    // tooltip
    var tooltip = $(
      '.js-mytooltip[data-mytooltip-id="' +
        $(this).parents("div").last().data("mytooltip-id") +
        '"]'
    );
    tooltip.myTooltip(
      "updateContent",
      $(this).closest(".wrap").clone().wrap("<div>").parent().html()
    );
  });
});
