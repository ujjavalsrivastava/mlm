...
(function (root, factory) {
  ...
  var TableExport = function (selectors, options, isUpdate) {
     ...
     */
    self.settings = isUpdate
      ? options
      : $.extend({}, TableExport.prototype.defaults, options);
    /**
    ...
    var rowD = TableExport.prototype.rowDel,
      ignoreRows =
        self.settings.ignoreRows instanceof Array
          ? self.settings.ignoreRows
          : [self.settings.ignoreRows],
      ignoreCols =
        self.settings.ignoreCols instanceof Array
          ? self.settings.ignoreCols
          : [self.settings.ignoreCols],
      ignoreCSS =
        self.settings.ignoreCSS instanceof Array
          ? self.settings.ignoreCSS.join(", ")
          : self.settings.ignoreCSS,
      emptyCSS =
        self.settings.emptyCSS instanceof Array
          ? self.settings.emptyCSS.join(", ")
          : self.settings.emptyCSS,
      bootstrapClass,
      bootstrapTheme,
      bootstrapSpacing;
    ...
    self.selectors.each(function () {
      ...
      var $rows = $el.find("tbody").find("tr"),
        $rows = self.settings.headings
          ? $rows.add($el.find("thead>tr"))
          : $rows,
        $rows = self.settings.footers ? $rows.add($el.find("tfoot>tr")) : $rows,
        thAdj = self.settings.headings ? $el.find("thead>tr").length : 0,
        fileName =
          self.settings.fileName === "id"
            ? $el.attr("id")
              ? $el.attr("id")
              : TableExport.prototype.defaultFileName
                    ...
                    $cols
                      .map(function (ic, val) {
                        ...
                        if (rcMap[ir]) {
                          ...
                          for (
                            var i = 0;
                            i <= Math.max.apply(Math, Object.keys(rcMap[ir]));
                            i++
                          ) {
                            !rcMap[ir][i]
                              ? count++
                              : (total =
                                  count >= ic ? total + rcMap[ir][i] : total);
                            if (count === threshold) {
                              break;
                            }
                          }
                ...
                .map(function (i, val) {
                  ...
                  return $cols
                    .map(function (i, val) {
                      ...
                      return (
                        '"' +
                        formatValue($(val).text().replace(/"/g, '""')) +
                        '"'
                      );
                    })
                    .get()
    ...
    $("button[data-fileblob]")
      ...
      .on("click", function () {
        ...
        TableExport.prototype.export2file(
          data,
          mimeType,
          fileName,
          fileExtension
