    ...
    jTLine: function (options) {
      ...
      var setTimelineWidth = function (timelineComponents, width) {
        ...
        if (options.distanceMode == "auto") {
          ...
          // Define Fixed Distance
          var fixedDistByPixl =
            options.eventsMinDistance * options.fixDistanceValue;
      ...
      var checkMQ = function () {
        ...
        return window
          .getComputedStyle(document.querySelector(".jtline"), "::before")
        ...
        if (options.callType == callTypes.ajax) {
          $.getJSON(options.url, function (data) {
            ...
            .done(function (data) {
              ...
              // console.log(data);
              // dataResult = data;
              if (data) _constructDom(data, obj);
              else {
      ...
      var _constructDom = function (jsonData, mainObject) {
        ...
        var dataCollection;
        if (options.map.dataRoot == "/") dataCollection = jsonData;
        else dataCollection = jsonData[options.map.dataRoot];
        ...
        for (var i = 0; i < dataCollection.length; i++) {
          ...
          $liTimeLineCollection +=
            '<li><a href="#0" data-date="' + point.dateValue + '" ';
          // if (point.isSelected) $liTimeLineCollection += 'className="selected"';
          $liTimeLineCollection +=
            'className="tlnode ' + (isSelectedPoint ? "selected" : "") + '"';
          ...
          if (isSelectedPoint)
            $liEventContentCollection += 'className="selected"';
          $liEventContentCollection +=
            ' data-date="' +
            point.dateValue +
            '">' +
            formatTitle(point.title, dataCollection[i]) +
            formatSubTitle(point.subTitle, dataCollection[i]) +
            formatBodyContent(point.bodyCnt, dataCollection[i]) +
            "</li>";
        }
        //-----------------------------------------
        ...
        var $divStart = '<div id="jtlinediv" className="jtline">';
        var $timeline =
          '<div className="timeline"><div className="events-wrapper"><div className="events"><ol>' +
          $liTimeLineCollection +
          '</ol><span className="filling-line" aria-hidden="true"></span></div></div><ul className="cd-timeline-navigation"><li><a href="#0" className="prev inactive">Prev</a></li><li><a href="#0" className="next">Next</a></li></ul></div>';
        var $eventsContent =
          '<div className="events-content"><ol>' +
          $liEventContentCollection +
          "</ol></div>";
        var $divEnd = "</div>";

        var allHtml = $divStart + $timeline + $eventsContent + $divEnd;
        $container.html(allHtml);
        // -----------------------------------------

        //------------------------------------------
        var timelineObj = $("#jtlinediv", $container);
      ...
      var formatTitle = function (title, obj) {
        ...
        if (formatTitleResult) return options.formatTitle(title, obj);
        else return "<h2>" + title + "</h2>";
      };

      var formatSubTitle = function (subTitle, obj) {
        var formatSubTitle = options.formatSubTitle(subTitle, obj);
        if (formatSubTitle) return options.formatSubTitle(subTitle, obj);
        else return "<em>" + point.subTitle + "</em>";
      };

      var formatBodyContent = function (bodyCnt, obj) {
        var formatBodyCnt = options.formatBodyContent(bodyCnt, obj);
        if (formatBodyCnt) return options.formatBodyContent(bodyCnt, obj);
        else return "<p>" + bodyCnt + "</p>";
      };
      ...
      return this.each(function () {
        ...
        // ------------------------
        var obj = $(this);
        // var timelineObj = ("#jtlinediv",this)
